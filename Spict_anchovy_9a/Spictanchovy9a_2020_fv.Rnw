...
library(devtools)
library(repmis)
#setwd("~/SPiCt/spict_2020")
setwd("~/Back up de MIPC/Documentos/TEXdocuments/SPICT9a_South/spict2020/lastbranch_092020")

#install_github("mawp/spict/spict") esta no
#install_github("tokami/spict/spict", ref="tvp_blim") #esta si, es la seasonal branch
#install_github("tokami/spict/spict", ref="seaprodTVP")
...
# #######################################
#fit1 está en un folder local porque es muy grande y el github no lo admite! Folder local: 
#setwd("~/Back up de MIPC/Documentos/TEXdocuments/SPICT9a_South/spict2020")
#setwd("~/Back up de MIPC/Documentos/TEXdocuments/SPICT9a_South/spict2020/lastbranch_092020")
...
pdf("data.pdf",width=5.27, height=5.27)
 ...
 ggsave("dataplot.jpg",plotspict.data(anchovy9aSouth), width = 5.27, height = 5.27, units = c("in", "cm", "mm"))
...
#  ############################
 load("fit3a.Rdata")
pdf("scenario3out.pdf",width=5, height=5)
plotspict.biomass(fit3a)
# #   # #print(sp1)
 dev.off()

pdf("scenario3outbbmsy.pdf",width=5, height=5)
plotspict.bbmsy(fit3a)
# #   # #print(sp1)
 dev.off()
...
Most important outputs for scenario 3 are displayed in figures \ref{scenario3out} and \ref{scenario3outbbmsy} . This scenario assumes no seasonal productivity, time-varying growth and uses the whole data set available. Diagnostics are displayed in figure \ref{diag3} and the following is the results summary:
...
\begin{figure}[h!]
 \centering
 \includegraphics[]{./scenario3outbbmsy.pdf}
 % lendist.pdf: 667x841 pixel, 72dpi, 23.53x29.67 cm, bb=0 0 667 841
 \caption{Summary of SPiCt results for scenario 3}
 \label{scenario3outbbmsy}
\end{figure}
